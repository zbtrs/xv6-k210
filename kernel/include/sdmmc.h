...
typedef uint32 (*SDMMC_EVWAIT_FUNC_T)(void);

/* Function prototype for milliSecond delay function */
typedef void (*SDMMC_MSDELAY_FUNC_T)(uint32);

/**
 * @brief SD/MMC Card specific setup data structure
 */
typedef struct {
	uint32 response[4];						/*!< Most recent response */
	uint32 cid[4];							/*!< CID of acquired card  */
	uint32 csd[4];							/*!< CSD of acquired card */
	uint32 ext_csd[512 / 4];					/*!< Ext CSD */
	uint32 card_type;							/*!< Card Type */
	uint16 rca;								/*!< Relative address assigned to card */
	uint32 speed;								/*!< Speed */
	uint32 block_len;							/*!< Card sector size */
	uint64 device_size;						/*!< Device Size */
	uint32 blocknr;							/*!< Block Number */
	uint32 clk_rate;							/*! Clock rate */
 ...
 */
#define CMDRESP_R3_OCR_VAL(n)           (((uint32) n) & 0xFFFFFF)
#define CMDRESP_R3_S18A                 (((uint32) 1 ) << 24)
#define CMDRESP_R3_HC_CCS               (((uint32) 1 ) << 30)
#define CMDRESP_R3_INIT_COMPLETE        (((uint32) 1 ) << 31)

/**
 * @brief R6 response definitions
 */
#define CMDRESP_R6_RCA_VAL(n)           (((uint32) (n >> 16)) & 0xFFFF)
#define CMDRESP_R6_CARD_STATUS(n)       (((uint32) (n & 0x1FFF)) | \
										 ((n & (1 << 13)) ? (1 << 19) : 0) | \
										 ((n & (1 << 14)) ? (1 << 22) : 0) | \
										 ((n & (1 << 15)) ? (1 << 23) : 0))
...
#define CMDRESP_R7_CHECK_PATTERN(n)     (((uint32) n ) & 0xFF)
/** Voltage accepted */
#define CMDRESP_R7_VOLTAGE_ACCEPTED     (((uint32) 1 ) << 8)

/**
 * @brief CMD3 command definitions
 */
/** Card Address */
#define CMD3_RCA(n)         (((uint32) (n & 0xFFFF) ) << 16)

/**
 * @brief CMD7 command definitions
 */
/** Card Address */
#define CMD7_RCA(n)         (((uint32) (n & 0xFFFF) ) << 16)

/**
 * @brief CMD8 command definitions
 */
/** Check pattern */
#define CMD8_CHECKPATTERN(n)            (((uint32) (n & 0xFF) ) << 0)
/** Recommended pattern */
#define CMD8_DEF_PATTERN                    (0xAA)
/** Voltage supplied.*/
#define CMD8_VOLTAGESUPPLIED_27_36     (((uint32) 1 ) << 8)

/**
 * @brief CMD9 command definitions
 */
#define CMD9_RCA(n)         (((uint32) (n & 0xFFFF) ) << 16)

/**
 * @brief CMD13 command definitions
 */
#define CMD13_RCA(n)            (((uint32) (n & 0xFFFF) ) << 16)

/**
 * @brief APP_CMD command definitions
 */
#define CMD55_RCA(n)            (((uint32) (n & 0xFFFF) ) << 16)

/**
 * @brief ACMD41 command definitions
 */
#define ACMD41_OCR(n)                   (((uint32) n) & 0xFFFFFF)
#define ACMD41_S18R                     (((uint32) 1 ) << 24)
#define ACMD41_XPC                      (((uint32) 1 ) << 28)
#define ACMD41_HCS                      (((uint32) 1 ) << 30)

/**
 * @brief ACMD6 command definitions
 */
#define ACMD6_BUS_WIDTH(n)              ((uint32) n & 0x03)
