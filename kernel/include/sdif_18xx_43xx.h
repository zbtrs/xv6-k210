...
typedef struct {				/*!< SDMMC Structure        */
	...
	uint32  GPIO;		/*!< General Purpose Input/Output Register */
...
typedef uint32 (*MCI_IRQ_CB_FUNC_T)(uint32);

/** Function prototype for SD detect and write protect status check */
typedef int (*PSCHECK_FUNC_T)(void);

/** Function prototype for SD slot power enable or slot reset */
typedef void (*PS_POWER_FUNC_T)(int enable);

/** @brief  SDIO chained DMA descriptor
 */
typedef struct {
	volatile uint32 des0;						/*!< Control and status */
	volatile uint32 des1;						/*!< Buffer size(s) */
	volatile uint32 des2;						/*!< Buffer address pointer 1 */
	volatile uint32 des3;						/*!< Buffer address pointer 2 */
} pSDMMC_DMA_T;
...
typedef struct _sdif_device {
	...
	/* uint32 sdio_clk_rate; */
	/* uint32 sdif_slot_clk_rate; */
	/* int clock_enabled; */
} sdif_device;
 ...
 * @return	None
 */
static inline void Chip_SDIF_SetBlkSize(LPC_SDMMC_T *pSDMMC, uint32 bytes)
{
	pSDMMC->BLKSIZ = bytes;
}
 ...
 */
static inline void Chip_SDIF_Reset(LPC_SDMMC_T *pSDMMC, int reset)
{
	if (reset) {
		pSDMMC->RST_N = 1;
	}
 ...
 */
static inline int Chip_SDIF_CardNDetect(LPC_SDMMC_T *pSDMMC)
{
 ...
 */
static inline int Chip_SDIF_CardWpOn(LPC_SDMMC_T *pSDMMC)
{
 ...
 * @return	None
 */
static inline void Chip_SDIF_SetCardType(LPC_SDMMC_T *pSDMMC, uint32 ctype)
{
	pSDMMC->CTYPE = ctype;
}
 ...
 */
static inline uint32 Chip_SDIF_GetIntStatus(LPC_SDMMC_T *pSDMMC)
{
 ...
 * @return	None
 */
static inline void Chip_SDIF_ClrIntStatus(LPC_SDMMC_T *pSDMMC, uint32 iVal)
{
	pSDMMC->RINTSTS = iVal;
}
 ...
 * @return	None
 */
static inline void Chip_SDIF_SetIntMask(LPC_SDMMC_T *pSDMMC, uint32 iVal)
{
	pSDMMC->INTMASK = iVal;
}
 ...
 * @return	None
 */
static inline void Chip_SDIF_SetBlkSizeByteCnt(LPC_SDMMC_T *pSDMMC, uint32 blk_size)
{
 ...
 * @return	None
 */
static inline void Chip_SDIF_SetByteCnt(LPC_SDMMC_T *pSDMMC, uint32 bytes)
{
	pSDMMC->BYTCNT = bytes;
}
 ...
 */
int Chip_SDIF_SendCmd(LPC_SDMMC_T *pSDMMC, uint32 cmd, uint32 arg);
 ...
 * @return	None
 */
void Chip_SDIF_GetResponse(LPC_SDMMC_T *pSDMMC, uint32 *resp);
 ...
 * @return	None
 */
void Chip_SDIF_SetClock(LPC_SDMMC_T *pSDMMC, uint32 clk_rate, uint32 speed);
 ...
 * @return	None
 */
void Chip_SDIF_DmaSetup(LPC_SDMMC_T *pSDMMC, sdif_device *psdif_dev, uint32 addr, uint32 size);
